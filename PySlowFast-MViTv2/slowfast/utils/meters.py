...
class TestMeter(object):
    ...
    def finalize_metrics(self, ks=(1, 5)):
        ...
        if self.multi_label:
            ...
            # print("Hehe: ", self.video_preds.shape, self.video_labels.shape)
            _, x_video_preds = torch.max(self.video_preds, 1)
            x_video_labels = self.video_labels
            np_video_preds = x_video_preds.detach().cpu().numpy()
            np_video_labels = x_video_labels.detach().cpu().numpy()
            cm = sklearn.metrics.confusion_matrix(np_video_labels, np_video_preds)
            disp = sklearn.metrics.ConfusionMatrixDisplay(confusion_matrix=cm)
            disp.plot()

            cm_idx_plot_path = os.path.join("cm_figure", str(cm_idx_plot) + ".png")
            plt.savefig(cm_idx_plot_path)
            cm_idx_plot += 1
