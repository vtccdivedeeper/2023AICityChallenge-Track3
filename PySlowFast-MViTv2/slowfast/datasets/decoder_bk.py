...
# by junwei: decord is said to be faster than pyav
def decord_decode(
    container,
    sampling_rate,
    num_frames,
    ...
    for k in range(len(num_frames)):
        ...
        frames = None
        frames = container.get_batch(np.arange(video_start_pts, video_end_pts + 1)).asnumpy()
...
def decode(
    ...
    if backend == "pyav":
        ...
        frames_decoded, fps, decode_all_video, start_end_delta_time = decord_decode(
            container,
            sampling_rate,
            num_frames,
            clip_idx,
