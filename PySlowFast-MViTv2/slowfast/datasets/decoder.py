...
#                 )
#                 frames = []
#                 stride = sampling_rate[k] // 3
#                 # print(stride)
#                 # print("Hehe: ", type(video_frames))
#                 gray_video_frames = []
#                 print(len(video_frames))
#                 for i in range(len(video_frames)):
#                     video_frames[i] = video_frames[i].to_rgb().to_ndarray()
#                     gray_video_frames.append(cv2.cvtColor(video_frames[i], cv2.COLOR_BGR2GRAY))
#                 for i in range(len(video_frames)):
#                     if i < len(video_frames) - sampling_rate[k]:
#                         # print("HEHE: ", video_frames[i].to_ndarray().shape, video_frames[i].to_gray().to_ndarray().shape, type(video_frames[i]))
#                         # for j in range(3):
#                         frame_1 = gray_video_frames[i]
#                         frame_2 = gray_video_frames[i + stride]
#                         frame_3 = gray_video_frames[i + 2*stride]
#                         frame_rgb = np.stack([frame_1, frame_2, frame_3], 2)
#                         # cv2.imwrite("images/test{}.png".format(count), frame_rgb)
#                         # count += 1
#                     else:
#                         frame_rgb = video_frames[i]
#                         # print("HIHI: ", frame_rgb.shape)
