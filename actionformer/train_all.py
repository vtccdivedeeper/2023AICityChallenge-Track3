...
def main(args):
    ...
    ckpt_folder_root = ckpt_folder

    # modify path for each fold/view
    for json_file in cfg['json_files']:
        cfg['dataset']['json_file'] = json_file
        if cfg['use_concat_view']:
            fold_name = json_file.split("/")[-2]
            ckpt_folder = os.path.join(ckpt_folder_root, fold_name)
        else:
            fold_name = json_file.split("/")[-2]
            view_name = json_file.split("/")[-3]
            ckpt_folder = os.path.join(ckpt_folder_root, f"{fold_name}{view_name}")
